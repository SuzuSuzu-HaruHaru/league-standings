...
In this remarkable group, each team won, drew, and lost exactly one match, leading to the final standings. What left many perplexed was Belgium’s position: their first-place spot was curiously taken away through a technicality in the rules. This incident occurred just eight years after the group discussed in the previous example, where, once again, Belgium finished as runners-up. The confusion stems from the fact that, with all teams tied on points (and thus requiring head-to-head comparisons across the entire table), goal difference should seemingly decide the outcome. Here, only Romania and Belgium remained tied, yet Romania ended up in first place, even though Belgium had won their head-to-head match 2-0 in the second round.
