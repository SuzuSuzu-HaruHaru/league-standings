...
For example, a *point* in standard soccer corresponds to three for a win, one for a draw, and zero for a loss. And although these terms are taken from soccer for simplicity, the principles apply to other sports as well, with only minor changes to the terminology.
